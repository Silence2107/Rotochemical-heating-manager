...
std::vector<double> tov_solver::tov_solution(std::vector<std::vector<double>> &cache, const std::function<double(double)> &eos_inv, double r, double center_pressure, double radius_step, double surface_pressure, size_t adaption_limit)
{
	...
	if (cache.empty())
	{
		...
		while (adaption_count < adaption_limit)
		{
			...
			m += adaptive_radius_step / 6 * (m_rk[0] + 2 * m_rk[1] + 2 * m_rk[2] + m_rk[3]);
			r += adaptive_radius_step;
			phi += 0.5 * (p - cache[2].back()) * (phi_integrand(p) + phi_integrand(cache[2].back())); // trapezoid integral
			// memorize results
			cache[0].push_back(r);
			cache[1].push_back(m);
			cache[2].push_back(p);
	...
	double mass = auxiliaries::math::interpolate(cache[0], cache[1], auxiliaries::math::InterpolationMode::kLinear, r);
	double pressure = auxiliaries::math::interpolate(cache[0], cache[2], auxiliaries::math::InterpolationMode::kLinear, r);
	double phi = auxiliaries::math::interpolate(cache[0], cache[3], auxiliaries::math::InterpolationMode::kLinear, r);
	return std::vector<double>({mass, eos_inv(pressure), phi, pressure, cache[0].back()});
}