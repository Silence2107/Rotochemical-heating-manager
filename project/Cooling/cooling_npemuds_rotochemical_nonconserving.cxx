...
int main(int argc, char **argv)
{
    ...
    auxiliaries::math::MatrixD zij(0, 0);
    try
    {
        zij = bij.inverse();
    }
    ...
    {
        RHM_THROW(std::runtime_error, "B-matrix inversion failed. " + std::string(e.what()));
    }
    ...
    for (auto species = constants::species::known_particles.begin(); species != constants::species::known_particles.end(); ++species)
    {
        if (i_omegas.find(*species) == i_omegas.end())
        {
            i_omegas[*species] = 0;
        }
    }
    ...
    auto rotochemical_vector_rhs = [&](double t, const std::vector<double> &funcs)
    {
        ...
        // create vector of etas with zero values for trivial etas; funcs[1+] are nontrivial \delta mu, ordered by rh_particles
        std::map<auxiliaries::phys::Species, double> dmus;
        ...
        for (auto species = known_particles.begin(); species != known_particles.end(); ++species)
        {
            auto pos = std::find(rh_particles.begin(), rh_particles.end(), *species);
            if (pos != rh_particles.end())
            {
                dmus[*species] = funcs[pos - rh_particles.begin() + 1];
            }
            else
            {
                dmus[*species] = 0;
        ...
        etas[electron] = dmus.at(neutron) - dmus.at(proton) - dmus.at(electron);
        etas[muon] = dmus.at(neutron) - dmus.at(proton) - dmus.at(muon);
        etas[uquark] = dmus.at(dquark) - dmus.at(uquark) - dmus.at(electron);
        etas[squark] = dmus.at(squark) - dmus.at(uquark) - dmus.at(electron);

        // rotochemical heating in npemuds
        auto diff_npl_inf = [&](double r, const auxiliaries::phys::Species &lepton)
        {
            return exp_phi(r) * (hadron_durca_rate_difference(r, lepton, t, T, etas.at(lepton)) +
        ...
        double rotochemical_luminosity = 0;
        for (auto it = particle_change.begin(); it != particle_change.end(); ++it)
        {
            rotochemical_luminosity -= it->second * dmus.at(it->first);
        }
