...
int main(int argc, char **argv)
{
    ...
    auto Q_nu = [&](double r, double t, double T, const std::map<std::string, double> &etas)
    {
        ...
        result += crust_plasma_emissivity(r, t, T);
        result += crust_eion_bremsstrahlung_emissivity(r, t, T);
        // quark processes
    ...
    // initial temperature profile. A bit overkill for RH, as profile is only used for surface temperature eval
    // so cleanup might be in order
