...
int main(int argc, char **argv)
{
        ...
        [&](std::vector<std::vector<double>> &cache, double r)
        {
            ...
            if (cache.empty())
            {
                ...
                for (size_t i = 0; i < cache[0].size(); ++i)
                {
                    ...
                    if (density_at_r < energy_density_conversion * edensity_core_limit && !crust_found)
                    {
                        crust_found = true;
                        r_crust = r_current;
                    }
    ...
    // We need Z_npe, I_e to perform test run
    // Z_npe is given (for now) by APR4 estimate for B_ee by Keisuke, in GeV
    double z_npe = 1E-60 * constants::conversion::erg_over_gev;
    ...
    auto integrand = [&](double r)
    {
        ...
        return -nbar_conversion * nbar(r) * 1.0 / omega_k_sqr * (Y_e(nbar(r + radius_step)) - Y_e(nbar(r))) / (tov(r + radius_step)[3] / tov(r)[3] - 1);
    };
